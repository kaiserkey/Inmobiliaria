...
public class RepositorioUsuario
{
    ...
    public List<Usuario> GetUsuarios(MySqlDatabase mySqlDatabase)
    {
        ...
        {
            cmd.CommandText = @"SELECT IdUsuario, Nombre, Apellido, Avatar, Email, Rol, Clave, Dni, Te FROM Usuario";
    ...
    public int UpdateUsuario(MySqlDatabase mySqlDatabase, Usuario usuario)
    {
        ...
        using (var cmd = mySqlDatabase.Connection.CreateCommand() as MySqlCommand)
        {
            cmd.CommandText = @"UPDATE Usuario SET Nombre=@Nombre, Apellido=@Apellido, Clave=@Clave, Avatar=@Avatar, Email=@Email, Rol=@Rol WHERE IdUsuario = @IdUsuario";
            cmd.Parameters.AddWithValue("@Nombre", usuario.Nombre);
            cmd.Parameters.AddWithValue("@Apellido", usuario.Apellido);
            cmd.Parameters.AddWithValue("@Clave", usuario.Clave);
            cmd.Parameters.AddWithValue("@Avatar", usuario.Avatar);
    ...
    public Usuario ObtenerPorEmail(MySqlDatabase mySqlDatabase, string email){
        ...
        using (var cmd = mySqlDatabase.Connection.CreateCommand() as MySqlCommand)
        {
            cmd.CommandText = @"SELECT IdUsuario, Nombre, Apellido, Clave, Avatar, Email, Rol FROM Usuario WHERE Email = @email";
